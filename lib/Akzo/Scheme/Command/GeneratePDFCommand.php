...
class GeneratePDFCommand extends \Symfony\Component\Console\Command\Command
{
    const COMMAND_NAME = 'generatePDF';
    ...
    protected function configure() {
        $this
            ...
            ->setDescription('Check if a scheme is approved, if yes generate dealer PDFs for it')
            //->addOption(
                //self::OPT_CODE,
                //self::OPT_CODE_SHORT,
    ...
    )
    {
        ...
        $actionToExecute = \Akzo\Scheme\CommandQueue::getInstance()->getNextSchemeAction(\Akzo\Scheme\Cache\KeyType::GENERATE_PDF);
        ...
        // $config = $this->app->getConfiguration()->getRawConfiguration(
        //     'pdf'
        // );
        $config = array();
        $config['path'] = '/pdf';
        ...
        // Execute the action
        //function generatePDFforDealers($schemeId, $path, $offset = 0, $count = 50)
        $result = \Akzo\Scheme\Service::getInstance()->generatePDFforDealers(
            $actionToExecute[\Akzo\Scheme\ExecuteActionAttribute::CODE],
            getcwd().$config['path'],
            $actionToExecute[\Akzo\Scheme\ExecuteActionAttribute::DEALER_OFFSET]
        );
        $GLOBALS['logger']->info("Got resulttttttt ".print_r($result, 1));
        ...
        if (isset($result['moreDealersLeft']) && $result['moreDealersLeft']) { 
            ...
            $GLOBALS['logger']->info("Removed Scheme Action fffffffffffffffffffffffffffffffffffffffffffff");
